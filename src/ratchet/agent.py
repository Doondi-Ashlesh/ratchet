"""Single-shot proposer: one prompt, one file back. It does not write, and it does
not judge.
...
Kept alongside the tool-calling agent in `coder`, not replaced by it. Measured over
ten files on four codebases the two accept the same number, and this one does it
for an eighth of the tokens, so which is the better default is a question the
benchmark answers rather than an assumption the code makes.
"""
...
def propose(path: str, diagnostics: Sequence[Classified], feedback: str = "") -> Proposal:
    ...
    reply = llm.chat_model().invoke([HumanMessage(prompt)])
    if llm.finish_reason(reply) == "length":
        e = llm.Truncated(
            f"{llm.model_id()} hit its output limit; the file is likely too large to "
            f"return whole"
        )
    ...
    # `.text` is a property; older versions exposed it as a method. Falling back to
    # `.content` keeps this working across both without calling the deprecated form.
    text = getattr(reply, "text", None)
    raw = text if isinstance(text, str) else str(reply.content)
